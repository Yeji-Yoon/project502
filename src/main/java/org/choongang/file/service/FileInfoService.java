...
public class FileInfoService {
    ...
    public FileInfo get(Long seq) {
        FileInfo fileInfo = repository.findById(seq).orElseThrow(FileNotFoundException::new);//예외 던짐
     ...
     *               ALL : 기본값 - 완료, 미완료 모두 조회
     *               DONe: 완료된 파일(true)
     *               UNDONE : 미완료된 파일(false)
     * @return
     */
    public List<FileInfo> getList(String gid, String location, String mode) {
        ...
        builder.and(fileInfo.gid.eq(gid)); //필수
    ...
    public List<FileInfo> getList(String gid) {
        return getList(gid, null,"ALL");//location이 null일때
    }

    public List<FileInfo> getList(String gid, String location) {
        return getList(gid, location,"ALL");//location이 null이 아닐때
    }

    public List<FileInfo> getListDone(String gid) {
        return getList(gid, null,"DONE");//DON : 완료된 파일
    }
    ...
    public void addFileInfo(FileInfo fileInfo) {
        ...
        if(_thumbDir.exists()){//디렉토리가 있을때만 가져옴
     ...
     * @return
     */
    public String[] getThumb(long seq, int width,int height) {//썸네일 가져옴.
        FileInfo fileInfo = get(seq);
        String fileType = fileInfo.getFileType();//파일이 이미지 인지 체크
